...
import type { CardItem } from '../../views/blocks'
...
export function FeatureCard({ item }: { item: CardItem }) {
  return (
    ...
    >
      ...
      {item.label ? (
        <span className="inline-flex w-fit items-center rounded-xl bg-accent-600/10 px-3 py-1 text-xs font-bold uppercase tracking-[0.12em] text-accent-700">
          {item.label}
        </span>
      ) : null}
      <h3
