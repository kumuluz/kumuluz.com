...
export function AccentCard({ item }: { item: CardItem }) {
  return (
    <motion.article
      ...
      variants={revealItem}
    >
      <span className="block h-1.5 w-8 rounded-sm bg-accent-600 transition-[width] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:w-16" />
      {item.label ? (
        <p className="mt-5 text-xs font-bold uppercase tracking-[0.14em] text-accent-700">
          {item.label}
        </p>
      ) : null}
      <h3
