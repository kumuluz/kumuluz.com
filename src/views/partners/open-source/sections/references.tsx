      ...
      <RevealGroup className="mt-12 flex flex-wrap gap-5">
        {block.items.map((item) => (
          <motion.article
            ...
            variants={revealItem}
          >
            <span className="pointer-events-none absolute -right-10 -top-10 size-28 rounded-full bg-accent-600/5 transition-transform duration-500 group-hover:scale-150" />
            <span className="inline-flex w-fit items-center rounded-full bg-accent-600/10 px-3 py-1 text-xs font-bold uppercase tracking-[0.12em] text-accent-700">
              {item.label}
            </span>
            ...
            <div className="relative mt-6">
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {item.expertise.map((tag) => (
                  <span
                    className="rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-medium text-neutral-600 transition-colors duration-200 group-hover:bg-accent-50 group-hover:text-accent-700"
                    key={tag}
                  >
                    {tag}
                  </span>
