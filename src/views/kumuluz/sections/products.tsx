...
import { dropdownLinkHref } from '../../../lib/expertise-links'
    ...
    <Section bg={bg} id="products">
      ...
      <RevealGroup className="mt-12 flex flex-wrap gap-5">
        {block.items.map((item) => (
          <motion.article
            className={`group flex grow basis-full flex-col rounded-xl border border-neutral-200 bg-white p-7 lg:basis-[calc((100%-1.25rem)/2)] ${cardHover}`}
            key={item.title}
            variants={revealItem}
          >
            ...
            <div className="mt-6">
              ...
              <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1.5">
                {item.related.map((label) => {
                  const href = dropdownLinkHref(label, language)
